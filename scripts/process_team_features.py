...
TEAM_STATS = [
    ...
    # Per-60 rates (raw per-game — use cumulative versions for prediction)
    ...
    # PP/SH hits, giveaways, takeaways per60
...
# Strength stats to compute cumulative per-60 rates for
CUMULATIVE_RATE_STATS = {
    "ev": ["shots_on_goal_for", "shots_on_goal_against",
           "shot_attempts_for", "shot_attempts_against",
           "goals_for", "goals_against",
           "fenwick_for", "fenwick_against"],
    "pp": ["shots_on_goal_for", "shots_on_goal_against",
           "shot_attempts_for", "shot_attempts_against",
           "goals_for", "goals_against",
           "fenwick_for", "fenwick_against"],
    "sh": ["shots_on_goal_for", "shots_on_goal_against",
           "shot_attempts_for", "shot_attempts_against",
           "goals_for", "goals_against",
           "fenwick_for", "fenwick_against"],
}
...
def aggregate_xg(shot_df):
    ...
    xg_pivot["xgf_total"] = xg_pivot[[c for c in xg_pivot.columns if c.startswith("xgf_")]].sum(axis=1)
    xg_pivot["xga_total"] = xg_pivot[[c for c in xg_pivot.columns if c.startswith("xga_")]].sum(axis=1)
    ...
    xg_sog_pivot["xgf_sog_total"] = xg_sog_pivot[[c for c in xg_sog_pivot.columns if c.startswith("xgf_sog_")]].sum(axis=1)
    xg_sog_pivot["xga_sog_total"] = xg_sog_pivot[[c for c in xg_sog_pivot.columns if c.startswith("xga_sog_")]].sum(axis=1)

    result = xg_pivot.merge(xg_sog_pivot, on=["game_id", "team"], how="left").fillna(0)
    print(f"  xG aggregated for {len(result)} team-game rows")
    print(f"  CSD mean: {result['xgf_total'].mean():.2f}  xG SOG mean: {result['xgf_sog_total'].mean():.2f}")
    return result
...
def aggregate_goalie_stats(goalie_df):
    ...
    for player_id, g in goalie_df.groupby("player_id"):
        ...
        for i in range(len(g)):
            ...
            past_season = past[past["season"] == row["season"]]
            if len(past_season) > 0:
                ts = past_season["shots_against"].sum()
                tv = past_season["saves"].sum()
                row["save_pct_current_season"] = tv / ts if ts > 0 else None
            ...
            if len(past) > 0:
                ts = past["shots_against"].sum()
                tv = past["saves"].sum()
                row["save_pct_career"] = tv / ts if ts > 0 else None
...
def add_shot_funnel_rates(df):
    ...
    for strength in ["ev", "pp", "sh"]:
        ...
        if f"xgf_{strength}" in df.columns:
            new_cols[f"{strength}_xg_per_sog_for"] = df[f"xgf_{strength}"] / sog_for
        if f"xga_{strength}" in df.columns:
            new_cols[f"{strength}_xg_per_sog_against"] = df[f"xga_{strength}"] / sog_against
...
def build_matchup_features(df):
    ...
    cumulative_check = [c for c in matchup.columns if "cumulative" in c]
    print(f"  Cumulative per-60 rate columns: {len(cumulative_check)}")
...
def main():
    ...
    print("\nStep 5: Computing rolling averages + cumulative per-60 rates...")
